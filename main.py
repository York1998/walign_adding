...
"""
	如果是手动输入的，则用手动输入的形参来更新参数；
	如果不是手动输入的，则默认读取文件里面的参数配置。
"""
if args.use_config:
	try:
		import json
...
""" 数据集名字，并规定噪声比例；
	之所以加入噪声，是在增加网络训练的难度，可以达到一定的正则效果，这是数据扩充的一个最简单的方式
	输入层注入噪声，其实可以看做是数据集增加的一种手段，本质是一种正则化
	输出层注入噪声，其实是对标签噪声及按摩，大部分数据集的label，总会有一定的错误率"""
